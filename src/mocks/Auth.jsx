...
class AuthApi{
    ...
    async updateUser(data,id){
      const response = await axios.put(`http://localhost:5000/userapp/user/update/${id}`,data,{
        method: "put",
        headers: { 
        "Authorization": `Bearer ${localStorage.getItem("accessToken")}`
        }
    ...
    async deleteUser(id){
      const response = await axios.delete(`http://localhost:5000/userapp/user/delete/${id}`,{
        method: "delete",
        headers: { 
        "Authorization": `Bearer ${localStorage.getItem("accessToken")}`
        }
   ...
   async ForgetPass(data){
    const responce = await axios.post(`http://localhost:5000/userapp/auth/reset-password-otp`,data);
